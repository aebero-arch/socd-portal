...
interface Props {
  pap: Pap;            // locked — cannot change PAP from here
  onClose: () => void;
  onSuccess: () => void;
}

export default function AddActivityModal({ pap, onClose, onSuccess }: Props) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Inject the locked PAP id
    formData.set("pap_id", pap.id);

    // Clear irrelevant period fields
  ...
  const inputCls =
    "w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-accent/20 transition-all font-body";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-xl bg-surface border border-border shadow-2xl tick-corners">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            ...
            <p className="text-xs text-ink-400 mt-0.5 line-clamp-1">
              PAP: <span className="font-medium text-ink">{pap.name}</span>
            </p>
          </div>
        ...
        <form id="add-activity-form" onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
          {/* Activity Type */}
